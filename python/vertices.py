        ...
        for face in description["faces"]:
            for i in range(0, len(face) - 1):
                ...
                if matrix is not None:
                    x1, y1, z1 = np.dot(matrix[:3, :3], np.array([x1, y1, z1]).T) + matrix[:3, 3]
                    x2, y2, z2 = np.dot(matrix[:3, :3], np.array([x2, y2, z2]).T) + matrix[:3, 3]
...
def match(gtdoc: Drawing, tgdoc: Drawing, layerlist: List[str] = list()):
    ...
    origin: np.ndarray = np.full((515, 512, 3), 255, dtype=np.uint8)
    origin = plot(gtstruct, gtdoc, 512, 512, (255, 0, 0), origin)
    origin = plot(tgstruct, tgdoc, 512, 512, (0, 0, 255), origin, matrix)
